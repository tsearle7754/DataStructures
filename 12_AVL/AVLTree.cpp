...
void AVLTree<T>::insert(const T& val, AVLNode<T>*& node) {
    ...
    node->height = std::max(getHeight(node->left), getHeight(node->right)) + 1;     // height of current node as well
...
void AVLTree<T>::remove(const T& val, AVLNode<T>*& node) {
    ...
    else if (node->hasTwoChildren()) {      // find minimum at right, then replace with node we are deleting, then remove the minimum from right
